...
> 
> 1. 打开终端，执行命令 `node agent-wait.cjs`。
...
> 5. **【上下文溢出自愈协议】** 如果你判断当前会话上下文即将耗尽导致你可能出现幻觉，请输出 `[REQUEST_NEW_SESSION]` 要求人类开启新对话。但同时必须提示人类：“请在新对话的第一句话再次执行 `node agent-wait.cjs` 以恢复工作流”。
> 6. **【极度致命红线】** 提交完代码后，绝对不能退出对话！你必须立刻、马上再次执行 `node agent-wait.cjs` 进入下一轮休眠等待。
> 7. **【反越权红线】** `watchdog.cjs` 是人类负责的底层后台进程。如果你发现缺少 `bridge.md`，必须向人类报错要求其启动 Watchdog。**严禁你在终端自作主张执行 `node watchdog.cjs`，这是绝对禁止的越权行为。**
> 8. 一直循环上述过程，直到 `bridge.md` 的状态变为 `COMPLETED`，你才能正式结束本次工作。
>
> 听懂了吗？听懂了请立即执行第一步：`node agent-wait.cjs`。
