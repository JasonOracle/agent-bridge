...
- **严禁直接在前台运行常驻服务**：禁止在前台直接运行 `npm run dev` / `pnpm dev` / `vite` 等常驻挂起进程。若需验证，必须编写超时自动杀进程的测试脚本或在后台运行。
- **包管理器兼容与加速**：根据项目锁文件自动选择 `npm` / `pnpm`。依赖安装推荐加上国内镜像源参数（如 `--registry=https://registry.npmmirror.com`）。
