...
> ⚠️ **防幻觉与防阻塞机制提醒**：
> `watchdog.js` 底层已经配置了 `devTimeoutMin` (开发超时剔除) 和重试上限。如果你遇到无法解析 JSON 导致死循环、或者 Builder 一直挂起的情况，请绝对保持静默，不要恐慌。底层脚本会自动将状态从 `IN_DEV` 或 `IN_REVIEW` 超时回退并推向 `ERROR` 或 `NEEDS_HUMAN` 以熔断死锁。作为 Supervisor，你只需要在收到正确的 `IN_REVIEW` 时干活即可。
